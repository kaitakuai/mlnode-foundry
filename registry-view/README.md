...
One JSON per published Stage 4 image, written by CI after `cosign sign`. The
...
| `tools/stage3.lock.cue` | `cuda` |
| `tools/naming.cue` | `name`, `tag` |
| Stage 4 build outputs (CI-supplied) | `digest`, `size`, `cosign_identity`, `slsa_attestation_url` |
| `git log` | `uploaded_by` |
...
The file is regenerated on every Stage 4 publish. Do NOT edit by hand —
edit `profiles/<x>.cue`, `tools/model-registry.cue`, or `tools/stage3.lock.cue`
and let CI re-emit.
