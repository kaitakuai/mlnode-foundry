...
h100_glm_5_3_flash: #OverlayProfile & bases.H100 & {
	...
	env: {
		...
		// dead, since dapi omits batch_size and MLNode fills its own 32 in. 8 on 8xH100: batch 16 crashed the engine before start on 2026-09-14
		// (XID 31, Vlad on 3.1.0; Crash_Bash_FL saw it once in several runs), 8 ran
		// every time. 32 is OOM here. H200 stays at 16.
		// Crash_Bash_FL, 2026-09-10.
		POC_BATCH_SIZE_DEFAULT: "8"
	}
	runtime_defaults: {
	...
	description: "H100 Hopper SXM5 (x8) + GLM-5.3-Flash FP8 (TP=8, fp8 KV, autotune off, gmu 0.95) - vllm-poc 0.28 PLUGIN, overlay on gonka-ai/mlnode 3.1.0"
	notes: """
