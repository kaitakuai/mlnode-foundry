...
| `poc-batch-size-from-env` | Rewrites MLNode's `batch_size: int = 32` defaults in `pow_v2_routes.py` to read `POC_BATCH_SIZE_DEFAULT`. Without it the env knob is dead on the production path: dapi omits `batch_size` on purpose, MLNode fills 32 in and forwards it explicitly, so the plugin never reaches its own default. Falls back to 32 when the variable is unset | Any profile that sets `POC_BATCH_SIZE_DEFAULT` |
