...
| `flashinfer-0-6-18-stable` | Same as the nightly pin but from the v0.6.18 stable release (2026-08-29), whose GitHub assets carry all three distributions including the cubin PyPI lacks. Ordering constraint identical. | GLM-5.3-Flash profiles since k3 |
...
| `drop-stock-ubuntu-user` | Removes the stock `ubuntu` account (UID 1000) that Ubuntu 24.04 bases ship, so mlnode's `entrypoint.sh` can create `appuser` at the default `HOST_UID=1000` instead of dying on `useradd: UID 1000 is not unique`. No-op on 22.04 bases | **Retired 2026-09-12**: gonka#1751 stopped the entrypoint from creating the account at all. Kept in `GONKA_BASE_PATCHES` for overlays on 3.0.16 and earlier; the GLM-5.3 leaves on 3.0.17 drop it |
| `poc-batch-size-force` | Rewrites both PoC proxy payloads in `pow_v2_routes.py` to overwrite `batch_size` with `POC_BATCH_SIZE_DEFAULT`, so the image outranks the caller. Without it the variable is dead on the production path: `gonka_poc` honours it only when the request omits `batch_size`, dapi omits it on purpose, and MLNode fills its own hardcoded 32 in. Leaves the request in charge when the variable is unset | Any profile that sets `POC_BATCH_SIZE_DEFAULT` |
| `replay-max-tokens-pin-revert` | Removes the replay `max_tokens` pin (kaitakuai/vllm#21) from the residual's `chat_completion/serving.py`, the Stage-4 form of gonka-ai/vllm#111. The pin rewrote `SamplingParams` after validation, so a client request with `min_tokens` above the recorded length killed the engine's input thread while `/health` stayed green (two GLM-5.3-Flash hangs, 2026-09-16). No-op once the base is past #111 | Bases built before #111 only; the GLM-5.3-Flash leaves moved to a base with #111 the same day and dropped it |
