...
| `replay-max-tokens-pin-revert` | Removes the replay `max_tokens` pin (kaitakuai/vllm#21) from the residual's `chat_completion/serving.py`, the Stage-4 form of gonka-ai/vllm#111. The pin rewrote `SamplingParams` after validation, so a client request with `min_tokens` above the recorded length killed the engine's input thread while `/health` stayed green (two GLM-5.3-Flash hangs, 2026-09-16). No-op once the base is past #111 | GLM-5.3-Flash leaves on bases built before #111 |
