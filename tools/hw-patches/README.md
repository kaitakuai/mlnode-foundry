...
| `drop-stock-ubuntu-user` | Removes the stock `ubuntu` account (UID 1000) that Ubuntu 24.04 bases ship, so mlnode's `entrypoint.sh` can create `appuser` at the default `HOST_UID=1000` instead of dying on `useradd: UID 1000 is not unique`. No-op on 22.04 bases. Same line belongs in upstream `mlnode/packages/api/Dockerfile` | Any overlay on a 24.04-based mlnode (`3.0.17` on); in `GONKA_BASE_PATCHES` |
