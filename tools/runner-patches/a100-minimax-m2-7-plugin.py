"""A100 MiniMax-M2.7 plugin hardcodes — 0.25.1 line, params inherited from the 2026-05 vLLM 0.20 campaigns (NOT yet revalidated on 0.25.1).
...
Usage inside a Dockerfile RUN step:
    COPY tools/runner-patches/a100-minimax-m2-7-plugin.py /tmp/a100-minimax-m2-7-plugin.py
    RUN python3 /tmp/a100-minimax-m2-7-plugin.py && rm /tmp/a100-minimax-m2-7-plugin.py
"""
...
INJECTION_LINES = [
    "",
    "# --- Kaitaku A100-MiniMax plugin hardcodes (tools/runner-patches/a100-minimax-m2-7-plugin.py) ---",
    "_a100_minimax_plugin_forced = {",
    ...
    "    '--logprobs-mode': 'processed_logprobs',",
    "}",
    "for _flag, _value in _a100_minimax_plugin_forced.items():",
    ...
    "_a100_minimax_plugin_defaults = {",
    "    '--max-model-len': '131072',",
    "    '--tool-call-parser': 'minimax_m2',",
    "    '--reasoning-parser': 'minimax_m2_append_think',",
    "    '--kv-cache-dtype': 'fp8',",
    "}",
    "for _flag, _value in _a100_minimax_plugin_defaults.items():",
    ...
    "# --- end Kaitaku A100-MiniMax plugin hardcodes ---",
]
...
def main() -> int:
    ...
    if MARKER not in src:
        sys.stderr.write(
            f"ERROR: Kaitaku A100-MiniMax plugin patch: marker {MARKER!r} not found in {FILE}. "
    ...
    if MODULE_MARKER not in src and not already_swapped:
        sys.stderr.write(
            f"ERROR: Kaitaku A100-MiniMax plugin patch: launch-module line {MODULE_MARKER!r} "
            f"not found in {FILE}. Upstream runner.py may have been refactored — re-verify "
            "the MLNODE_VLLM_MODULE swap.\n"
        )
        return 1

    already_injected = "Kaitaku A100-MiniMax plugin hardcodes" in src
    ...
    if not already_injected:
        ...
        if not inserted:
            sys.stderr.write(
                "ERROR: Kaitaku A100-MiniMax plugin patch: marker present but insertion did not fire\n"
            )
            return 1
        out = "".join(new_lines)
    ...
    if not already_swapped:
        if MODULE_MARKER not in out:
            sys.stderr.write(
                "ERROR: Kaitaku A100-MiniMax plugin patch: launch-module marker vanished before swap\n"
            )
            return 1
    ...
    with open(FILE, "w") as f:
        f.write(out)
    print(
        "runner.py patched for A100-MiniMax plugin hardcodes + MLNODE_VLLM_MODULE launch swap"
    )
    return 0
