"""H100 MiniMax-M2.7 DECODE-PoC plugin hardcodes for MLNode runner.py.

Per-card decode variant (campaign 2026-08-15/16, migration-025-kit/PROFILES.md).
Same two structural edits as h100-minimax-decode-plugin.py. Forces the decode
launch profile for 4xH100: TP=4, moe=triton, mns 704,
compilation-config {"max_cudagraph_capture_size":608}
(capture 608 per the wall>512 rule).
gpu-memory-utilization 0.90: STRICTLY 0.90: at 0.92 vLLM over-provisions KV (389k tokens) and the PoC graph capture OOMs (~768 MiB short) wedging the engine — forced-overwrite, a governance 0.92 broadcast must NOT win here.
...
INJECTION_LINES = [
    "",
    "# --- Kaitaku H100-MiniMax DECODE plugin hardcodes (tools/runner-patches/h100-minimax-decode-plugin.py) ---",
    "_h100_minimax_decode_forced = {",
    ...
    '''    '--compilation-config': '{"max_cudagraph_capture_size":608}',''',
    "}",
    "for _flag, _value in _h100_minimax_decode_forced.items():",
    ...
    "_h100_minimax_decode_defaults = {",
    "    '--tool-call-parser': 'minimax_m2',",
    "    '--reasoning-parser': 'minimax_m2_append_think',",
    "    '--kv-cache-dtype': 'fp8',",
    "}",
    "for _flag, _value in _h100_minimax_decode_defaults.items():",
    ...
    "# --- end Kaitaku H100-MiniMax DECODE plugin hardcodes ---",
]
...
def main() -> int:
    ...
    if MARKER not in src:
        sys.stderr.write(
            f"ERROR: Kaitaku H100-MiniMax DECODE plugin patch: marker {MARKER!r} not found in {FILE}. "
    ...
    if MODULE_MARKER not in src and not already_swapped:
        sys.stderr.write(
            f"ERROR: Kaitaku H100-MiniMax DECODE plugin patch: launch-module line {MODULE_MARKER!r} "
    ...
    if not already_injected:
        ...
        if not inserted:
            sys.stderr.write(
                "ERROR: Kaitaku H100-MiniMax DECODE plugin patch: marker present but insertion did not fire\n"
            )
            return 1
        out = "".join(new_lines)
    ...
    if not already_swapped:
        if MODULE_MARKER not in out:
            sys.stderr.write(
                "ERROR: Kaitaku H100-MiniMax DECODE plugin patch: launch-module marker vanished before swap\n"
            )
            return 1
    ...
    with open(FILE, "w") as f:
        f.write(out)
    print(
        "runner.py patched for H100-MiniMax DECODE plugin hardcodes + MLNODE_VLLM_MODULE launch swap"
    )
    return 0
