...
The Stage 4 build resolves to `tools/runner-patches/<name>.py`, COPYs it into the image at `/tmp/runner-patches/<name>.py`, runs it, and removes it.

Every patcher inserts its forced-args block after the same anchor line,
`MARKER = "self.processes: List[subprocess.Popen] = []"` — the first line of
`VLLMRunner.__init__` that mlnode has left untouched across the 0.25.1
release candidates. The `self.additional_args = ...` assignment above it is
not usable as an anchor: rc3 rewrote it to inject a default
`--worker-extension-cls`. A patcher exits non-zero when its anchor is
missing, and Stage 4 fails the build on that.
...
| `h200-kimi-k2-6-plugin.py` | H200 (×8, Hopper sm_90) Kimi-K2.6 INT4 on the release base: TP=8, gmu=0.90, max-model-len=240000, **FLASHMLA** (not CUTLASS_MLA — that is sm_100+), kimi_k2 parsers, mm-encoder-tp-mode=data, expert-parallel, trust-remote-code, enable-auto-tool-choice, processed_logprobs, `--worker-extension-cls`. Every serving flag is gonka's `node-config-kimik26-H200.json` verbatim — the only Kimi tuning that exists for Hopper. Deliberately does NOT force `--compilation-config` (so `--enforce-eager` is left alone) or any batch sizing: both Blackwell leaves set those from measured envelopes, and no H200 Kimi benchmark exists. |
...
## LEGACY (fat-fork lineage, not referenced by any profile — do not wire into new profiles)

These patchers target the legacy fat-fork monolith base and are kept only
because tuning_notes source strings cite their paths. Deletion (with
git-permalink repointing of those sources) is deferred until images are
validated on real GPU hardware.
...
| `b200-kimi-k2-6.py` | B200 Kimi-K2.6 INT4 (rev=2 tune): forces TP=4, gpu_memory_utilization=0.93, max_num_batched_tokens=32768, max_model_len=120000, cudagraph FULL_AND_PIECEWISE (superseded by `b200-kimi-k2-6-plugin.py`) |
