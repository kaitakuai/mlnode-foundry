...
| `b200-glm-5-2-plugin.py` | B200 (×8) GLM-5.2 FP8 on the vllm-poc PLUGIN base: swaps the launch module to `MLNODE_VLLM_MODULE` and forces the operator GLM-5.2 tune (TP=8, gpu-memory-utilization=0.85, max-model-len=350000, max-num-batched-tokens=16384, max-num-seqs=16, kv-cache-dtype=fp8_e4m3, glm47/glm45 parsers, `--compilation-config {"mode":0,"cudagraph_mode":"NONE"}` eager, processed_logprobs, trust-remote-code, enable-auto-tool-choice) plus `--worker-extension-cls gonka_poc.worker.PoCWorkerExtension`. REMOVES `--enforce-eager` (conflicts with `--compilation-config`; eager is set via mode=0). Does NOT pin `--attention-backend` (GLM-5.2 is DSA, not MLA — no CUTLASS_MLA) and does NOT force expert-parallel. triton MoE is forced off-DeepGEMM via the `GLM_5_2` profile base env. |
