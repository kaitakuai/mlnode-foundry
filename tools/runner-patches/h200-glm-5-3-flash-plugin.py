"""H200 GLM-5.3-Flash PLUGIN-base hardcodes for MLNode runner.py.
...
(kaitakuai/experiments/2026-09). 4x141 GB, TP=4; the 4xH200 measurement ran with the defaults otherwise (66 min, 101k nonces, fp8 KV).
...
INJECTION_LINES = [
    "",
    "# --- Kaitaku H200-GLM-5.3-Flash plugin hardcodes (tools/runner-patches/h200-glm-5-3-flash-plugin.py) ---",  # noqa: E501
    "_h200_glm53_forced = [",
    ...
    "]",
    "for _flag, _value in _h200_glm53_forced:",
    ...
    "for _flag, _value in _h200_glm53_defaults:",
    "    if _flag not in self.additional_args:",
    "        self.additional_args.extend([_flag, _value])",
    "for _flag in _h200_glm53_flags:",
    "    if _flag not in self.additional_args:",
    "        self.additional_args.append(_flag)",
    "# --- end Kaitaku H200-GLM-5.3-Flash plugin hardcodes ---",
]
...
def main() -> int:
    ...
    if MARKER not in src:
        sys.stderr.write(
            "ERROR: h200-glm-5-3-flash patch: forced-args marker not found. "
            "Upstream runner.py may have been refactored - re-verify the patch.\n"
        )
        return 1
    if "_h200_glm53_forced" in src:
    ...
    if MODULE_MARKER in src:
        ...
        sys.stderr.write(
            "ERROR: h200-glm-5-3-flash patch: launch-module marker not found and no "
