...
The actual bottleneck is **push bandwidth** — 45 GB to GHCR per Stage 3 publish (~10-15 min). Build time itself is dominated by `docker pull` of Stage 1+2 base layers (already cached on GHCR CDN).
...
`mlnode-foundry hash <profile>` computes SHA-256 over (profile.cue + naming.cue + hw_patches/ files referenced + runner_patch + Stage 2 digest from `stage2.lock.cue`). On `build-stage3.yml` execution, compare against `gonka.kaitaku.profile_hash` label on the current GHCR tag. Match → skip rebuild, retag.

**No remote cache services** (no BuildBuddy, no Depot). Justified by ADR-0008 + the bandwidth-bottleneck observation above.

## Retention

- Stage 1 / Stage 2 tags: keep ≥ 3 months for traceability
- Stage 3 tags: **keep indefinitely** (public artifact pinned by node operators, experiments)
